...
- **背景自定义**：输入框右上角的图片图标按钮（选择图片 / 清除 / 透明度滑块），托盘菜单同步可用；配置存于应用数据目录，卸载保留
...
聊天输入框右上角的麦克风图标按钮（与背景按钮同一工具栏，随输入框定位、继承 dsh 主题色）：按住开始听，松开把识别文本插入输入框。
...
- **麦克风设备**：WinRT 识别器固定使用系统**「默认通信设备」**的麦克风（Windows 设置 → 系统 → 声音 → 更多声音设置 → 录制 → 设为默认通信设备），与平时默认输入设备不同时会提示。蓝牙耳机在音乐模式下麦克风静音，需处于通话/佩戴状态才有声音。
...
- **输入框工具栏**：语音/背景按钮行插到 `button[class$="_primary"]`（发送按钮）前，`locateRow` 从 textarea 向上找该按钮做锚点；dsh 改发送按钮类名即失效
